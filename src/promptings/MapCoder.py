...
class MapCoder(BaseStrategy):
    ...
    def run_single_pass(self, data_row: dict):
        ...
        # Check if this is LiveCodeBench dataset
        if self.is_livecodebench():
            # Enhanced prompt for LiveCodeBench with competitive programming focus
            input_kb_exemplars = [
                {
                    "role": "user",
                    "content": f"""Given a competitive programming problem from LiveCodeBench, provide relevant problems then identify the algorithm behind it and also explain the tutorial of the algorithm.

# Problem:
{self.data.get_prompt(data_row)}

# Exemplars:
Recall {mapping[self.k]} relevant and distinct competitive programming problems (different from problem mentioned above). For each problem,
1. describe it briefly
2. identify the algorithm category (greedy, dynamic programming, graph, etc.)
3. generate {self.language} code step by step to solve that problem
4. finally generate a planning to solve that problem
...
<root>
<problem>
# Recall {mapping[self.k]} relevant and distinct competitive programming problems. Write each problem in the following format.
<description>
...
""",
            ...
            # Original prompt for other datasets
            input_kb_exemplars = [
                {
                    "role": "user",
